...
const funcs = require("./BLACK/funcs/utils.js");
global.funcs = funcs;
...
const langFile = (readFileSync(`${__dirname}/languages/${global.config.langs || "en"}.lang`, { encoding: 'utf-8' })).split(/\r?\n|\r/);
...
try {
  var appStateFile = resolve(join(global.client.mainPath, global.config.APPSTATEPATH || "BlackCookies.json"));
...
const DB_GryKJ = await require('./DB/controller/index.js')(loginApiData);
    ...
    global.client.timeStart = new Date().getTime(),
      function() {
        const listCommand = readdirSync(global.client.mainPath + '/BLACK/cmds').filter(command => command.endsWith('.js') && !command.includes('example') && !global.config.commandDisabled.includes(command));
        for (const command of listCommand) {
          try {
            var module = require(global.client.mainPath + '/BLACK/cmds/' + command);
            ...
            if (module.xPrefix) global.client.eventRegistered.push(module.config.name);
            global.client.commands.set(module.config.name, module);
            if (module.config.KJ) {
            const Yora = module.config.KJ;
            for (let piro of Yora) {
              global.client.KJ.set(piro, module);
            }
          }
            ...
            logger.log([
              ...
              {
                message: `Can't load module : ${module.config.name} 
                ${error}`,
                color: "white",
              },
            ]);
      ...
      function() {
        const events = readdirSync(global.client.mainPath + '/BLACK/evts').filter(event => event.endsWith('.js') && !global.config.eventDisabled.includes(event));
        for (const ev of events) {
          try {
            var event = require(global.client.mainPath + '/BLACK/evts/' + ev);
            if (!event.config || !event.onType) throw new Error(global.getText('mirai', 'errorFormat'));
